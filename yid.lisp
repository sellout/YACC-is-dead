...
(defgeneric derive (parser value)
  ...
  (:method ((parser con) value)
    (if (nullablep (left parser))
        (alt (con (derive (left parser) value) (right parser))
             (con (eps (parse-null (left parser)))
  ...
  (:method ((language red))
    (let ((l2 (slot-value language 'parser)))
      (cond ((emptyp l2) *empty*)
            ((nullablep l2)
             (eps (mapcar (slot-value language 'function) (parse-null l2))))
